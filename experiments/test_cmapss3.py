...
def run(kind, fd, seeds=5):
    data = load_cmapss(r"E:\datasets\C-MAPSS", fd, seed_rolling=False)
    print(fd, kind, 'features', data['x_train'].shape[2])
    vals, tests = [], []
    for seed in range(1, seeds+1):
        set_seed(seed)
        if kind == 'proposed':
            m = build_model(data['x_train'].shape[1], data['x_train'].shape[2], dropout_rate=0.15, seed=seed)
        else:
            m = build_deep_baseline(kind, data['x_train'].shape[1], data['x_train'].shape[2], seed=seed)
        train_model(m, data['x_train'], data['y_train'], data['x_val'], data['y_val'], seed=seed)
