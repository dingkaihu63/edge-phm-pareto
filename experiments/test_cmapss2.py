import numpy as np
from common_no_tf import calibrate_threshold, evaluate_binary
from prepare_data import load_cmapss
from torch_common import build_model, mc_predict, set_seed, train_model

def run(name, fd, per_engine_std, seeds=2):
    data = load_cmapss(r"E:\datasets\C-MAPSS", fd, seed_rolling=False, per_engine_std=per_engine_std)
    print(name, fd, 'features', data['x_train'].shape[2])
    vals, tests = [], []
    for seed in range(1, seeds+1):
        set_seed(seed)
        m = build_model(data['x_train'].shape[1], data['x_train'].shape[2], lstm_units_1=96, lstm_units_2=48, dropout_rate=0.1, seed=seed)
        train_model(m, data['x_train'], data['y_train'], data['x_val'], data['y_val'], lr=1e-3, batch_size=64, seed=seed)
        pv,_ = mc_predict(m, data['x_val'], samples=50)
        pt,_ = mc_predict(m, data['x_test'], samples=50)
        vals.append(pv); tests.append(pt)
    pv = np.mean(vals,0); pt = np.mean(tests,0)
    th,_ = calibrate_threshold(data['y_val'], pv)
    r = evaluate_binary(data['y_test'], pt, th)
    print(name, fd, 't', round(th,2), 'acc/prec/rec/f2/auc/pr', round(r['accuracy'],3), round(r['precision'],3), round(r['recall'],3), round(r['f2'],3), round(r['auc_roc'],3), round(r['auc_pr'],3))

run('per_std','FD001', True)
run('global_std','FD001', False)
run('per_std','FD003', True)
run('global_std','FD003', False)
